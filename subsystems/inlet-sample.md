...
- **User field report (2026-08-11):** A core facility that has since **migrated all of its MC-ICP-MS methods to the Isotope2** reported back on the same thread: tuning became far more routine, stability "routinely rivaling or exceeding that of self-aspiration", and low volumes and slow injection are both covered. The poster stated no commercial relationship with the vendor and offered to discuss privately.
- **Credit:** asked by Bernd Hoeppner; answered by Paul Field (Elemental Scientific); field report by Tyler Goepfert (ASU METAL Core)
